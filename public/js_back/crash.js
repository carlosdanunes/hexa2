$(document).ready(function() {
    ...
    this.publishTime = (res) => {
        $('#chartInfo').text('Старт через ' + res.value + 'с.');
    }
    ...
    this.reset = (res) => {
        $('#bets').html('');
        $('#chartInfo').css('color', '#ffc645').text('Загрузка');
    ...
    this.parseSlider = (res) => {
        ...
        $('#chartInfo').text(((res.crashed) ? 'Стоп на ' : '') + 'x' + res.float.toFixed(2));
        if(res.crashed) 
        {
            ...
            if(!window.isCashout && window.withdraw > 0) $('.btn-withdraw span').text('Вывести ' + parseFloat(window.bet*parseFloat(res.float.toFixed(2))).toFixed(2));
    ...
    $('.btn-play').click(() => {
        $.ajax({
            ...
            success : res => {
                ...
                {
                    window.bet = res.bet;
                    $('.btn-withdraw span').text('Вывести ' + (window.bet).toFixed(2));
