                            ...
                            <div class="row">
                                <div class="col col-3 col-md-6">
                                    <div class="counter-block">
                                        <div class="counter-num">{{$stats['countUsers'] + 20000}}</div>
                                ...
                                <div class="col col-3 col-md-6">
                                    <div class="counter-block">
                                        <div class="counter-num">{{$stats['totalGames'] + 20000}}</div>
                                ...
                                <div class="col col-3 col-md-6">
                                    <div class="counter-block">
                                        <div class="counter-num white">
                                            <div class="bet-number"><span class="bet-wrap"><span>{{$stats['totalWithdraw'] + 20000}}</span>
					...
					<div class="wrap">
						...
						<div class="deposit-section tab active" data-type="deposite">
							<form action="/pay" method="post" id="payment">
								...
								<div class="form-row">
									<label>
										<div class="form-label big">Пополняй счет на сумму от {{$settings->dep_bonus_min}}р. и получай бонус +{{$settings->dep_bonus_perc}}% на свой баланс!</div>
									</label>
								</div>
								@endif
								<div class="form-row">
						...
						<div class="faucet-modal-give {{$gv->status == 1 ? 'doneGive' : ''}}" id="gv_{{$gv->id}}">
							<div class="give-btn-block">
								<div class="faucet-reload">
									<div class="faucet-cd">Раздача #{{$gv->id}}</div>
								</div>
							</div>
							<div class="row">
								<div class="col-6">
									<div class="bank-text">Сумма:</div>
									<div class="bank-amount">{{$gv->sum}} <svg class="icon icon-coin {{$gv->type}}"><use xlink:href="/img/symbols.svg#icon-coin"></use></svg></div>
								</div>
								<div class="col-6">
									<div class="date-text">До начала:</div>
									<div class="date-of">{{$gv->status == 0 ? $gv->time_to : 'Проведена'}}</div>
								</div>
							</div>
							@if($gv->status == 0)
								...
								<div class="give-btn-block nowGive">
									<div class="faucet-reload">
										...
										<div class="text-left">
											@if($gv->group_sub != 0 && $settings->vk_url)<div class="faucet-sm-text">• Быть подписанным на нашу <a href="{{$settings->vk_url}}" target="_blank">группу VK</a>.</div>@endif
											@if($gv->min_dep != 0)<div class="faucet-sm-text">• Пополнить счет на сумму {{$gv->min_dep}}р. за текущий день.</div>@endif
										</div>
									</div>
								</div>
								@endif
								<div class="give-btn-block nowGive">
									<button type="button" class="btn btn-green joinGiveaway" data-id="{{$gv->id}}"><span>Учавствовать</span></button>
									<div class="faucet-sm-text">Участников: <span class="total_users">{{$gv->total}}</span></div>
								</div>
							@endif
							<div class="give-btn-block">
								<div class="faucet-reload">
			...
			<div class="modal-content">
				...
				<div class="tos-modal-container">
					<div class="scrollbar-container tos-modal-block ps">
						...
						<p>1.1. Настоящее соглашение (далее «Соглашение») регламентирует порядок и условия предоставления услуг сайтом «{{$settings->domain}}», именуемой далее по тексту «Организатор», и адресовано физическому лицу, желающему получать услуги указанного сайта (далее «Участник».)</p>
						...
						<p>1.3. Условия настоящего соглашения принимаются участниками в полном объеме и без каких-либо оговорок, путем присоединения к соглашению в том виде, в каком оно изложено на сайте «{{$settings->domain}}»</p>
						<h2>Термины и Определения</h2>
						<p>2.1. Предметом настоящего Соглашения является предоставление организатором участнику услуг по организации досуга и отдыха в игре «{{$settings->domain}}» в соответствии с условиями настоящего Соглашения. Под такими услугами, в частности, понимаются следующие: услуги по покупке-продаже игрового инвентаря ({{$settings->domain}}), ведение учета значимой информации: движения по игровому счету, обеспечение мер по идентификации и безопасности участников, разработка программного обеспечения, интегрируемого в игровую площадку и внешние приложения, информационные и другие услуги, необходимые для организации игры и обслуживания участника в ее процессе на площадке организатора.</p>
						...
						<p>2.3. Участник соглашается, что он несет персональную ответственность за все действия, произведенные с игровым инвентарем ({{$settings->domain}}): покупкой, продажей, вводом и выводом, а также за игровые действия на игровой площадке: создание, покупку-продажу, операции со всеми игровыми элементами и другими игровыми атрибутами и объектами, используемыми для игрового процесса.</p>
						...
						<p>3.1.1. Принимать участие в игре «{{$settings->domain}}» могут только лица, достигшие гражданской дееспособности по законодательству страны своей резиденции. Все последствия неисполнения данного условия возлагаются на участника.</p>
						...
						<p>4.1.1.2. вести учет игрового инвентаря ({{$settings->domain}}) на игровом счете участника.</p>
						...
						<p>4.1.1.6. Любому лицу, законно владеющему игровым инвентарем ({{$settings->domain}}), осуществляется оплата денежной суммы, обусловленной курсовой стоимостью ({{$settings->domain}}), за вычетом затрат на осуществление данной операции.</p>
